...
//const {data, loading, getData} = useGetMethods();
	...
	const {mutate: HandleDelete} = useMutation(
		 () =>{ return  DeleteMethod(`/api/method/${props.id}`)
		},
		 {onSuccess: async () => {
		 	console.log("success")
								...
								<div>
									<Stack direction='row' alignItems='center'>
										{!props.data?.isMethodSet ? (
											<SvgIcon color='primary' viewBox='0 -6 24 24'>
										...
										<Typography sx={{ fontSize: 11, fontWeight: "500", textAlign: "left" }} color='#000'>
											{props?.data?.isMethodSet ? "Methodset" : "Method"}
										</Typography>
									</Stack>
								</div>
								<div>
									<ButtonGroup disableElevation variant='contained' aria-label='Disabled elevation buttons'>
										...
										{user?.data?.userId === props.data?.ownerId ? (
											<>
												<Link to={`/EditMethod/${props?.data?.id}`} state={{ prefill: props.data, methodUpdate: true }} component={<MethodEditForm />}>
